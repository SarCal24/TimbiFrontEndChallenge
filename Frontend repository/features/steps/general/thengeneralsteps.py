...
from hamcrest import equal_to, assert_that, only_contains,contains_string
...
@then(u'The url page should contains the next "(?P<url>.*)"  in the url')
def step_contains_url(context, url):
    GeneralComponents.wait_until_url_contains(context.browser, url)
    return assert_that(context.web_driver.current_url, contains_string(url))
